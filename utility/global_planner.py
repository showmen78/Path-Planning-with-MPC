...
class RoutePlanSummary:
    ...
    maneuver_markers: List[Dict[str, object]] = field(default_factory=list)
    debug_reason: str = ""
...
class AStarGlobalPlanner:
    ...
    def _route_waypoints_from_carla_trace(
    ...
    ) -> tuple[List[List[float]], List[int], List[str], str, int | None, List[Dict[str, object]]]:
        route_waypoints: List[List[float]] = []
        route_lane_ids: List[int] = []
        road_options: List[str] = []
        maneuver_markers: List[Dict[str, object]] = []
        ...
        for carla_waypoint, road_option in route_trace:
            ...
            if len(route_waypoints) == 0 or math.hypot(
                ...
                if len(route_waypoints) > 0:
                    cumulative_progress_m += math.hypot(
                        float(waypoint_xy[0]) - float(route_waypoints[-1][0]),
                        float(waypoint_xy[1]) - float(route_waypoints[-1][1]),
                    )
            ...
            if option_name in {"LEFT", "RIGHT", "STRAIGHT"}:
                if len(maneuver_markers) == 0 or str(maneuver_markers[-1].get("road_option", "")) != str(option_name):
                    maneuver_markers.append(
                        {
                            "x_m": float(waypoint_xy[0]),
                            "y_m": float(waypoint_xy[1]),
                            "progress_m": float(cumulative_progress_m),
                            "road_option": str(option_name),
                            "lane_id": int(lane_id) if lane_id is not None else -1,
                        }
                    )
    ...
    @staticmethod
    def _nearest_progress_along_route_waypoints(
        ...
        for idx in range(len(route_waypoints) - 1):
            x0_m, y0_m = float(route_waypoints[idx][0]), float(route_waypoints[idx][1])
            x1_m, y1_m = float(route_waypoints[idx + 1][0]), float(route_waypoints[idx + 1][1])
            dx_m = x1_m - x0_m
            dy_m = y1_m - y0_m
            ...
            segment_length_m = math.sqrt(seg_len_sq)
            if distance_m < best_distance_m:
                best_distance_m = float(distance_m)
                best_progress_m = float(total_progress_m + proj * segment_length_m)
            total_progress_m += segment_length_m
        return float(best_progress_m), float(total_progress_m), float(best_distance_m)
    ...
    def _build_route_summary_from_carla_trace(
        ...
        route_waypoints, route_lane_ids, road_options, next_macro_maneuver, preferred_lane_id, maneuver_markers = self._route_waypoints_from_carla_trace(route_trace)
        if len(route_waypoints) == 0:
            return None
        ...
        return RoutePlanSummary(
            ...
            maneuver_markers=list(maneuver_markers),
    ...
    def summarize_cached_route_progress(
        self,
        cached_route_summary: RoutePlanSummary,
        start_xy: Sequence[float],
        goal_xy: Sequence[float] | None = None,
        max_projection_error_m: float = 15.0,
    ) -> RoutePlanSummary:
        if not bool(getattr(cached_route_summary, "route_found", False)):
            return cached_route_summary
        route_waypoints = list(getattr(cached_route_summary, "route_waypoints", []) or [])
        if len(route_waypoints) < 2:
            return cached_route_summary

        start_x_m = float(start_xy[0]) if len(start_xy) >= 1 else 0.0
        start_y_m = float(start_xy[1]) if len(start_xy) >= 2 else 0.0
        projected_progress_m, route_length_m, projection_error_m = self._nearest_progress_along_route_waypoints(
            route_waypoints=route_waypoints,
            xy=[float(start_x_m), float(start_y_m)],
        )
        if float(projection_error_m) > float(max_projection_error_m):
            return self.plan_route(
                start_xy=list(start_xy),
                goal_xy=list(goal_xy or route_waypoints[-1]),
            )

        goal_xy = list(goal_xy or route_waypoints[-1])
        goal_x_m = float(goal_xy[0]) if len(goal_xy) >= 1 else float(route_waypoints[-1][0])
        goal_y_m = float(goal_xy[1]) if len(goal_xy) >= 2 else float(route_waypoints[-1][1])

        start_query = self.nearest_waypoint_query(x_m=float(start_x_m), y_m=float(start_y_m))
        goal_query = self.nearest_waypoint_query(x_m=float(goal_x_m), y_m=float(goal_y_m))

        next_marker: Dict[str, object] | None = None
        for marker in list(getattr(cached_route_summary, "maneuver_markers", []) or []):
            if float(marker.get("progress_m", -1.0)) + 1e-6 >= float(projected_progress_m):
        ...
        if next_marker is None:
            ...
            optimal_lane_id = int(getattr(cached_route_summary, "goal_lane_id", getattr(cached_route_summary, "optimal_lane_id", -1)))
        else:
            road_options = [str(next_marker.get("road_option", ""))]
            next_macro_maneuver = self._next_macro_maneuver_from_road_options(road_options)
            optimal_lane_id = int(next_marker.get("lane_id", getattr(cached_route_summary, "optimal_lane_id", -1)))

        return RoutePlanSummary(
            route_found=True,
            start_road_id=str(getattr(start_query, "road_id", getattr(cached_route_summary, "start_road_id", "unknown_road"))),
            start_lane_id=int(getattr(start_query, "lane_id", getattr(cached_route_summary, "start_lane_id", -1))),
            goal_road_id=str(getattr(goal_query, "road_id", getattr(cached_route_summary, "goal_road_id", "unknown_road"))),
            goal_lane_id=int(getattr(goal_query, "lane_id", getattr(cached_route_summary, "goal_lane_id", -1))),
            optimal_lane_id=int(optimal_lane_id),
            distance_to_destination_m=float(max(0.0, float(route_length_m) - float(projected_progress_m))),
            next_macro_maneuver=str(next_macro_maneuver),
            route_waypoints=[],
            road_options=list(road_options),
            maneuver_markers=list(getattr(cached_route_summary, "maneuver_markers", []) or []),
            start_graph_index=int(getattr(start_query, "index", -1)),
            goal_graph_index=int(getattr(goal_query, "index", -1)),
            start_graph_xy=(
                float(getattr(start_query, "x_m", 0.0)),
