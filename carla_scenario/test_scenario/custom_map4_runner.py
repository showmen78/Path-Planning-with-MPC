"""
CARLA scenario runner for the custom_map4 test scene.
"""
...
from behavior_planner.global_planner import AStarGlobalPlanner
...
def _direction_key(carla_lane_id: int) -> str:
    return "positive" if int(carla_lane_id) > 0 else "negative"


def _round_xy(x_m: float, y_m: float) -> tuple[float, float]:
    return round(float(x_m), 3), round(float(y_m), 3)


def _carla_waypoint_group_key(waypoint) -> tuple[int, int, str]:
    return int(waypoint.road_id), int(waypoint.section_id), _direction_key(int(waypoint.lane_id))
...
def build_lane_center_waypoints(map_obj, carla, sample_distance_m: float) -> tuple[list[dict], dict]:
        ...
        valid_waypoints.append(
            {
                ...
                "direction": _direction_key(int(waypoint.lane_id)),
    ...
    waypoint_by_xy = {_round_xy(item["position"][0], item["position"][1]): item for item in valid_waypoints}
    for item in valid_waypoints:
        ...
        next_candidates = [
            ...
            and _direction_key(int(waypoint.lane_id)) == _direction_key(int(carla_waypoint.lane_id))
        ]
        if next_candidates:
            next_waypoint = min(
                next_candidates,
    ...
    *,
    ...
    while not stop_event.is_set():
        with shared_lock:
            ...
            previous_behavior = str(shared_state.get("current_applied_behavior", "LANE_KEEP"))
...
def run_loaded_world(client, world, scenario_cfg: Mapping[str, object], carla) -> int:
    ...
    try:
        ...
        while True:
            ...
            route_progress_m, _route_length_m = _nearest_progress_along_route(route_points, ego_state[:2])
            rolling_target_distance_m = _lookahead_distance_m(
                speed_mps=float(ego_state[2]),
                route_points=route_points,
                progress_m=float(route_progress_m),
                local_goal_cfg=local_goal_cfg,
            )
            route_based_destination_state = _temporary_destination_state(
                ego_state=ego_state,
                route_points=route_points,
                final_destination_state=final_destination_state,
                mpc_cfg=mpc_cfg,
                lookahead_distance_m=float(rolling_target_distance_m),
            )
            ...
            if str(current_applied_behavior).upper() not in {"EMERGENCY_BRAKE", "STOP_AT_LINE"}:
                current_target_v_mps = float(route_based_destination_state[2])
            ...
            if bool(behavior_runtime_cfg.get("enabled", False)):
                ...
                if (
                    ...
                    execution = apply_behavior_planner_decision(
                        ...
                        base_destination_state=route_based_destination_state,
                        final_destination_state=final_destination_state,
                        lane_center_waypoints=lane_center_waypoints,
                        selected_lane_id=int(current_lane_id),
            ...
            active_target_lane_id = int(current_lane_id)
            if behavior_upper in {"LANE_CHANGE_LEFT", "TURN_LEFT"}:
                active_target_lane_id = min(int(current_lane_id) + 1, int(lane_count))
            elif behavior_upper in {"LANE_CHANGE_RIGHT", "TURN_RIGHT"}:
                active_target_lane_id = max(1, int(current_lane_id) - 1)
            ...
            if behavior_upper in {"LANE_KEEP", "GO_STRAIGHT"}:
                temporary_destination_state = list(route_based_destination_state)
            else:
                temporary_destination_state = build_destination_on_lane(
                    ego_snapshot={
                        "x": float(ego_state[0]),
                        "y": float(ego_state[1]),
                        "v": float(ego_state[2]),
                ...
                if temporary_destination_state is None:
                    temporary_destination_state = list(route_based_destination_state)
            temporary_destination_state[2] = float(current_target_v_mps)

            final_lock_distance_m = float(local_goal_cfg.get("lock_to_final_distance_m", 30.0))
            final_distance_m = math.hypot(
                float(final_destination_state[0]) - float(ego_state[0]),
                float(final_destination_state[1]) - float(ego_state[1]),
            )
            if final_distance_m <= final_lock_distance_m:
                temporary_destination_state = list(final_destination_state)
            ...
            if display is not None and topdown_queue is not None and chase_queue is not None:
                ...
                temp_lane_prompt = prompt_builder._prompt_lane_id(
                    raw_lane_id=int(temp_lane_context.get("lane_id", 1)),
                    lane_count=lane_count,
                )
                lookahead_distance_m = math.hypot(
                    float(temporary_destination_state[0]) - float(ego_state[0]),
                    float(temporary_destination_state[1]) - float(ego_state[1]),
                )
                ...
                hud_lines = [
                    ...
                    f"lookahead_cmd={float(rolling_target_distance_m):.2f} m",
                    f"lookahead_actual={float(lookahead_distance_m):.2f} m",
                    f"decision={str(current_applied_behavior)}",
                ]
                topdown_overlay = None
                if topdown_camera is not None:
                    topdown_overlay = {
                        "camera_transform": topdown_camera.get_transform(),
