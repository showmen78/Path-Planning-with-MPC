...
def _collect_forward_reference_samples(
    ...
    lane_center_waypoints: Sequence[Mapping[str, object]],
    target_lane_id: int,
    sample_distances_m: Sequence[float],
    next_maneuver: str | None = None,
) -> List[List[float]]:
    ordered_targets = [
        (max(0.0, float(distance_m)), idx)
        for idx, distance_m in enumerate(sample_distances_m)
    ]
    if len(ordered_targets) == 0:
        return []

    outputs: List[List[float] | None] = [None] * len(ordered_targets)
    ordered_targets.sort(key=lambda item: float(item[0]))
    ...
    def _internal_waypoint_state(waypoint: Mapping[str, object], lane_id: int) -> List[float] | None:
        position = _position_of_waypoint(waypoint)
        if position is None:
            return None
        return [
            ...
            float(position[1]),
            0.0,
            float(waypoint.get("heading_rad", ego_psi_rad)),
            float(max(1, int(lane_id))),
        ]

    snap_lane_waypoints_cache: Dict[int, Sequence[Mapping[str, object]]] = {}

    def _carla_waypoint_state(carla_waypoint, lane_id_hint: int | None) -> List[float] | None:
        snap_lane_id = int(lane_id_hint) if lane_id_hint is not None else int(target_lane_id)
        if snap_lane_id not in snap_lane_waypoints_cache:
            snap_lane_waypoints_cache[snap_lane_id], _, _ = _lane_waypoints_by_lane_id(
                lane_center_waypoints=lane_center_waypoints,
                target_lane_id=int(snap_lane_id),
            )
        snap_lane_waypoints = snap_lane_waypoints_cache.get(snap_lane_id, [])
        snapped_state = _snap_carla_target_to_internal_lane_state(
            carla_target_waypoint=carla_waypoint,
            lane_waypoints=snap_lane_waypoints if len(snap_lane_waypoints) > 0 else lane_waypoints,
            ego_state=(ego_x_m, ego_y_m, 0.0, ego_psi_rad),
        )
        if snapped_state is not None:
        ...
        selected_transform = getattr(carla_waypoint, "transform", None)
        selected_location = getattr(selected_transform, "location", None)
        selected_rotation = getattr(selected_transform, "rotation", None)
        if selected_location is None:
            return None
        return [
            float(getattr(selected_location, "x", ego_x_m)),
            float(getattr(selected_location, "y", ego_y_m)),
            0.0,
            math.radians(float(getattr(selected_rotation, "yaw", math.degrees(ego_psi_rad)))),
            float(max(1, int(snap_lane_id))),
        ]

    def _fill_remaining_with_state(start_idx: int, state: List[float] | None) -> None:
        if state is None:
            return
        for idx in range(start_idx, len(ordered_targets)):
            target_order_idx = int(ordered_targets[idx][1])
            outputs[target_order_idx] = list(state)
    ...
    if best_segment is None:
        ...
        target_cursor = 0
        current_state = _internal_waypoint_state(current_waypoint, int(target_lane_id))
        while target_cursor < len(ordered_targets) and float(ordered_targets[target_cursor][0]) <= 1e-6:
            outputs[int(ordered_targets[target_cursor][1])] = list(current_state) if current_state is not None else None
            target_cursor += 1
        ...
        while target_cursor < len(ordered_targets):
            ...
            current_progress_m += float(segment_length_m)
            current_state = _internal_waypoint_state(current_waypoint, int(target_lane_id))
            while (
                target_cursor < len(ordered_targets)
                and float(ordered_targets[target_cursor][0]) <= float(current_progress_m) + 1e-6
            ):
                outputs[int(ordered_targets[target_cursor][1])] = list(current_state) if current_state is not None else None
                target_cursor += 1

        _fill_remaining_with_state(target_cursor, current_state)
        return [list(item) for item in outputs if item is not None]
    ...
    if current_carla_waypoint is not None and hasattr(current_carla_waypoint, "next"):
        ...
        local_segment_offset_m = max(0.0, float(current_alpha) * float(current_segment_length_m))
        absolute_targets = [
            (float(target_distance_m) + float(local_segment_offset_m), idx)
            for target_distance_m, idx in ordered_targets
        ]
        ...
        current_state = _carla_waypoint_state(current_carla_waypoint, last_internal_lane_id)
        while (
            target_cursor < len(absolute_targets)
            and float(absolute_targets[target_cursor][0]) <= float(current_progress_m) + 1e-6
        ):
            outputs[int(absolute_targets[target_cursor][1])] = list(current_state) if current_state is not None else None
            target_cursor += 1

        while target_cursor < len(absolute_targets):
            remaining_to_target_m = float(absolute_targets[target_cursor][0]) - float(current_progress_m)
            if remaining_to_target_m <= 1e-6:
                outputs[int(absolute_targets[target_cursor][1])] = list(current_state) if current_state is not None else None
                target_cursor += 1
                continue

            query_distance_m = min(2.0, float(remaining_to_target_m))
            try:
                next_candidates = [
                    candidate
                    for candidate in list(current_carla_waypoint.next(float(query_distance_m)))
            ...
            current_position_carla = _carla_waypoint_position(current_carla_waypoint)
            selected_position = _carla_waypoint_position(selected_carla_waypoint)
            if selected_position is None:
                break

            if current_position_carla is None:
                traveled_distance_m = float(query_distance_m)
            else:
                traveled_distance_m = max(
                    0.05,
                    _distance_between_points(current_position_carla, selected_position),
                )
            ...
            visited_positions.add(selected_key)
            current_progress_m += float(traveled_distance_m)
            while (
                target_cursor < len(absolute_targets)
                and float(absolute_targets[target_cursor][0]) <= float(current_progress_m) + 1e-6
            ):
    ...
    distance_to_segment_end_m = max(0.0, (1.0 - float(current_alpha)) * float(current_segment_length_m))
    target_cursor = 0
    current_progress_m = float(distance_to_segment_end_m)
    current_waypoint = next_waypoint
    current_state = _internal_waypoint_state(current_waypoint, int(target_lane_id))
    while (
        target_cursor < len(ordered_targets)
...
def build_lane_center_reference_to_destination(
    ...
    stage_distances_m = [
        min(
            float(clamped_target_distance_m),
            float(stage_idx) * float(sample_step_distance_m),
        )
    ...
    anchor_waypoint = _nearest_waypoint_on_lane(
        lane_center_waypoints=lane_center_waypoints,
    ...
    if anchor_waypoint is None:
        anchor_waypoint = _nearest_waypoint_any_lane(
            lane_center_waypoints=lane_center_waypoints,
            reference_x_m=float(ego_x_m),
            reference_y_m=float(ego_y_m),
        )
    if anchor_waypoint is not None:
        anchor_position = _position_of_waypoint(anchor_waypoint)
        if anchor_position is not None:
            reference_samples.append(
                {
                    "x_ref_m": float(anchor_position[0]),
                    "y_ref_m": float(anchor_position[1]),
                    "heading_rad": float(anchor_waypoint.get("heading_rad", ego_psi_rad)),
                    "lane_id": int(destination_lane_id),
                }
            )

    terminal_state = list(destination_state[:5])
    if len(terminal_state) < 5:
        terminal_state.append(float(destination_lane_id))

    stage_states = _collect_forward_reference_samples(
        ego_snapshot=ego_snapshot,
        lane_center_waypoints=lane_center_waypoints,
        target_lane_id=int(destination_lane_id),
        sample_distances_m=stage_distances_m,
        next_maneuver=next_maneuver,
    )
    if len(stage_states) == 0 and len(stage_distances_m) > 0:
        fallback_terminal_state = build_destination_on_lane(
            ego_snapshot=ego_snapshot,
            lane_center_waypoints=lane_center_waypoints,
            target_lane_id=int(destination_lane_id),
            target_distance_m=float(clamped_target_distance_m),
            road_cfg=road_cfg,
            next_maneuver=next_maneuver,
        )
        if fallback_terminal_state is not None:
            terminal_state = list(fallback_terminal_state)
        stage_states = [list(terminal_state) for _ in stage_distances_m]
    elif len(stage_states) < len(stage_distances_m):
        last_stage_state = list(stage_states[-1]) if len(stage_states) > 0 else list(terminal_state)
        while len(stage_states) < len(stage_distances_m):
            stage_states.append(list(last_stage_state))

    for stage_state in stage_states:
        reference_samples.append(
            {
                "x_ref_m": float(stage_state[0]),
                "y_ref_m": float(stage_state[1]),
                "heading_rad": float(stage_state[3]) if len(stage_state) >= 4 else float(ego_psi_rad),
                "lane_id": int(destination_lane_id),
            }
        )
