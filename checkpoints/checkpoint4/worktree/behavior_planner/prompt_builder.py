...
class BehaviorPlannerPromptBuilder:
    ...
    @staticmethod
    def _signal_phase_code(road_cfg: Mapping[str, object]) -> int:
        raw_value = road_cfg.get(
            "signal_phase",
            road_cfg.get("traffic_signal_phase", road_cfg.get("signal_state", 1)),
        )
        if isinstance(raw_value, str):
            lowered = str(raw_value).strip().lower()
            if lowered in {"red", "-1"}:
                return -1
            if lowered in {"yellow", "amber", "0"}:
                return 0
            if lowered in {"green", "1"}:
                return 1
        try:
    ...
    def build_context(
        ...
        return BehaviorPlannerPromptContext(
            ...
            distance_to_signal_m=float(road_cfg.get("distance_to_signal_m", 10000.0)),
            signal_phase_code=int(self._signal_phase_code(road_cfg)),
    ...
    def build_prompt(
        ...
        lines.extend([
            ...
            (
                ...
                f"{int(context.signal_phase_code)}]"
            ),
        ])
