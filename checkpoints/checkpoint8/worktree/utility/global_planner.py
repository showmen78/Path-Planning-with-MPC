...
class AStarGlobalPlanner:
    ...
    def __init__(
        ...
        self._lane_context_cache_xy: Tuple[float, float] | None = None
    ...
    def get_current_route_info(self, x_m: float, y_m: float) -> RoutePlanSummary:
        ...
        # Find nearest waypoint on the stored route (vectorised)
        dx = self._stored_route_xy[:, 0] - float(x_m)
        dy = self._stored_route_xy[:, 1] - float(y_m)
        nearest_idx = int(np.argmin(dx * dx + dy * dy))

        # Remaining route from ego's position onward
        remaining_options = self._stored_route_options[nearest_idx:]
        remaining_lane_ids = self._stored_route_lane_ids[nearest_idx:]
        remaining_xy = self._stored_route_xy[nearest_idx:]

        # Next macro maneuver from remaining per-waypoint options
        next_maneuver = self._next_macro_maneuver_from_road_options(remaining_options)

        # Optimal lane: first upcoming lane change target, else current lane
        optimal_lane_id = int(self._stored_route_summary.optimal_lane_id)
        for i, opt in enumerate(remaining_options):
            upper_opt = str(opt).upper()
            if upper_opt in {"LEFT", "RIGHT", "STRAIGHT", "CHANGELANELEFT", "CHANGELANERIGHT"}:
                if i < len(remaining_lane_ids) and int(remaining_lane_ids[i]) > 0:
                    optimal_lane_id = int(remaining_lane_ids[i])
                break
    ...
    def get_local_lane_context(
        ...
        with self._lane_context_lock:
            if self._lane_context_cache_xy is not None and self._lane_context_cache_result is not None:
                dx = float(x_m) - self._lane_context_cache_xy[0]
                dy = float(y_m) - self._lane_context_cache_xy[1]
                if (dx * dx + dy * dy) < self._lane_context_cache_threshold_m ** 2:
                    return dict(self._lane_context_cache_result)

        result = self._compute_local_lane_context(x_m=x_m, y_m=y_m, heading_rad=heading_rad)
        with self._lane_context_lock:
            self._lane_context_cache_xy = (float(x_m), float(y_m))
