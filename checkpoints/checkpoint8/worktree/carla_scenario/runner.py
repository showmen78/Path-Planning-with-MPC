...
def run_loaded_world(client, world, scenario_cfg: Mapping[str, object], carla) -> int:
    ...
    # Start with v_ref=0 (hold still) until the first behavior planner response.
    current_target_v_mps = 0.0
    received_first_behavior_response = False
    ...
    try:
        ...
        while True:
            ...
            if mpc.should_replan(sim_time_s):
                ...
                # Use stored route waypoints for the temporary destination so
                # it follows the correct branch at intersections (left/right/
                # straight) instead of re-deriving branches from lane-center
                # waypoints which can pick the wrong one.
                route_waypoints = list(getattr(current_route_summary, "route_waypoints", []) or [])
                if len(route_waypoints) >= 2:
                    base_destination_state = compute_temporary_destination_state_from_route(
                        ego_snapshot=ego_snapshot,
                        route_points=route_waypoints,
                    ...
                    base_destination_state = compute_temporary_destination_state(
                        ...
                        target_v_mps=float(current_target_v_mps),
                        road_cfg=road_cfg,
                        next_maneuver=str(getattr(current_route_summary, "next_macro_maneuver", "Continue Straight")),
                    )
                ...
                if bool(behavior_runtime_cfg.get("enabled", False)):
                    ...
                    if (
                        ...
                        if not received_first_behavior_response:
                            received_first_behavior_response = True
                            current_target_v_mps = float(original_max_velocity_mps)
                            print(f"[BEHAVIOR] First response received — v_ref set to v_max={float(original_max_velocity_mps):.2f} m/s")
                        else:
                ...
                if (
                    ...
                    if len(route_waypoints) >= 2:
                        temporary_destination_state = compute_temporary_destination_state_from_route(
                            ego_snapshot=ego_snapshot,
                            route_points=route_waypoints,
                ...
                # Use route-based reference when stored route is available;
                # this follows the correct branch at intersections.
                if len(route_waypoints) >= 2:
                    local_lane_center_reference = build_route_reference_samples(
                        ego_snapshot=ego_snapshot,
                        route_points=route_waypoints,
            ...
            if display is not None and topdown_queue is not None and chase_queue is not None:
                ...
                route_maneuver_code = prompt_builder._macro_maneuver_code(
                    str(getattr(current_route_summary, "next_macro_maneuver", "Continue Straight"))
                )
                ...
                temp_lane_prompt = prompt_builder._prompt_lane_id(
                    raw_lane_id=int(cached_hud_temp_lane_prompt),
                    lane_count=int(lane_count),
                )
                route_optimal_lane_id = int(getattr(current_route_summary, "optimal_lane_id", 1))
                ...
                hud_lines = [
                    ...
                    lane_line,
                    route_line,
                    f"gp_route_opt={str(getattr(current_route_summary, 'next_macro_maneuver', 'Continue Straight'))}",
