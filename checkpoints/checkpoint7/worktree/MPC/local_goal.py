...
def build_lane_center_reference_to_destination(
    ...
    terminal_state = build_destination_on_lane(
        ego_snapshot=ego_snapshot,
        lane_center_waypoints=lane_center_waypoints,
        target_lane_id=int(destination_lane_id),
    ...
    for stage_idx in range(1, int(horizon_steps) + 1):
        stage_distance_m = min(
            float(clamped_target_distance_m),
            float(stage_idx) * float(sample_step_distance_m),
        )
        stage_state = build_destination_on_lane(
            ego_snapshot=ego_snapshot,
            lane_center_waypoints=lane_center_waypoints,
            target_lane_id=int(destination_lane_id),
            target_distance_m=float(stage_distance_m),
            road_cfg=road_cfg,
            next_maneuver=next_maneuver,
        )
        if stage_state is None:
            stage_state = list(terminal_state)
