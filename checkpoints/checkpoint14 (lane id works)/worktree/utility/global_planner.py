...
class AStarGlobalPlanner:
    ...
    def get_current_route_info(
        ...
        # blue dot's closest position on the stored route.  We use the simplest
        # direct lookup: first non-INVALID lane id in the remaining sequence
        # starting at route_index.  The old approach (preparatory intersection
        # look-ahead) incorrectly included "STRAIGHT" intersections in its
        # turn-option set and scanned the entire remaining route, so any future
        # intersection where the route approach happens to be lane 1 caused
        # optimal_lane_id to collapse to 1 even when the route is currently in
        # lane 2.  The junction-freeze in runner.py handles the intersection
        # case: when ego_in_junction the last pre-junction optimal_lane_id is
        # held, so no look-ahead is needed here.
