...
class RuleBasedBehaviorPlanner:
    ...
    def __init__(
        ...
        self._latched_stop_target: Dict[str, Any] | None = None
        self._latched_signal_actor_id: object | None = None
    ...
    def update(
        ...
        Returns
        -------
        {"decision": "lane_follow" | "lane_change_left" | "lane_change_right" | "reroute" | "stop",
         "target_lane_id": int,
         "lc_state": str}
        """
        ...
        if len(reroute_messages) > 0:
            ...
            self._latched_signal_actor_id = None
            reroute_ids = [
        ...
        stop_result, traffic_light_debug = self._traffic_light_stop_result(
        ...
        if int(ego_lane_id) == int(route_lane_id):
            if route_lane_is_unsafe:
                ...
                if detour_lane_id is not None:
                    ...
                    if detour_lane_score > float(self._target_lane_safety_threshold):
                        return self._start_one_step_lane_change(
                            ego_lane_id=int(ego_lane_id),
        ...
        # Return to the route lane as soon as it is safe again. Normal mode
        # should immediately drive the blue dot back toward the route-optimal
        # lane with no additional hysteresis or cooldown.
        route_lane_safe_again = (
            route_lane_score >= float(self._optimal_lane_unsafe_threshold)
        )
    ...
    def _traffic_light_stop_result(
        ...
        *,
        ...
        if should_stop_now and isinstance(traffic_stop_target, Mapping):
            ...
            self._latched_stop_target = dict(traffic_stop_target)
            if bool(signal_found):
                self._latched_signal_actor_id = signal_actor_id
        elif (
            self._stop_latched
            and self._latched_stop_target is not None
            ...
            if (
                bool(signal_found)
                and (
                    self._latched_signal_actor_id is None
                    or signal_actor_id == self._latched_signal_actor_id
                )
            ):
                self._latched_signal_actor_id = signal_actor_id
        ...
        traffic_light_debug = {
            ...
            "stop_decision_active": bool(self._stop_latched),
            "latched_signal_actor_id": self._latched_signal_actor_id,
        }

        if not self._stop_latched or self._latched_stop_target is None:
            return None, traffic_light_debug

        target_lane_id = int(
            self._latched_stop_target.get("lane_id", int(ego_lane_id))
        )
        return (
            self._make_result(
                _DECISION_STOP,
                int(target_lane_id),
                stop_target=self._latched_stop_target,
                traffic_light_debug=traffic_light_debug,
            ),
            traffic_light_debug,
        )
