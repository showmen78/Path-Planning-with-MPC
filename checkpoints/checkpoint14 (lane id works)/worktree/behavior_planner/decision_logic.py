...
def _lane_count_from_inputs(
    ...
    for item in lane_center_waypoints:
        ...
        if lane_id == 0 or lane_id in lane_ids:
            continue
...
def apply_behavior_planner_decision(
    ...
    if decision is None:
        return BehaviorExecutionResult(
            ...
            selected_lane_id=int(selected_lane_id if selected_lane_id is not None else 0),
    ...
    current_lane_id = int(local_context.get("lane_id", 0))
    _ = _lane_count_from_inputs(
    ...
    if len(local_lane_ids) == 0 and int(current_lane_id) != 0:
        local_lane_ids = [int(current_lane_id)]
