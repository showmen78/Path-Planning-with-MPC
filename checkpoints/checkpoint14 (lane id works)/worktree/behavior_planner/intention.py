...
def infer_vehicle_intention(
    ...
    current_lane_id = int(current_context.get("lane_id", 0))
    ...
    if future_lane_id != current_lane_id:
        ordered_lane_ids = [
            int(lane_id)
            for lane_id in list(current_context.get("lane_ids", []))
            if int(lane_id) != 0
        ]
        if (
            int(current_lane_id) in ordered_lane_ids
            and int(future_lane_id) in ordered_lane_ids
        ):
            lane_code = (
                "L"
                if ordered_lane_ids.index(int(future_lane_id))
                > ordered_lane_ids.index(int(current_lane_id))
                else "R"
            )
        else:
