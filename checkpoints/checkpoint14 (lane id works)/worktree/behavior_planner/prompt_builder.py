...
class BehaviorPlannerPromptBuilder:
    ...
    def _lane_count_from_inputs(
        ...
        if len(lane_ids) == 0:
            return 1
        return len(list(dict.fromkeys(lane_ids)))

    @staticmethod
    def _prompt_lane_id(raw_lane_id: int, lane_count: int) -> int:
        """Expose the project's canonical lane ids directly in the prompt."""
        del lane_count
        return int(raw_lane_id)
    ...
    def _nearest_waypoint_on_lane(
        ...
        lane_waypoints = [
            waypoint
            for waypoint in lane_center_waypoints
            if int(waypoint.get("lane_id", 0)) == int(target_lane_id)
    ...
    def _lane_safe_flag(
        ...
        lane_exists = any(
            int(waypoint.get("lane_id", 0)) == int(target_lane_id)
            for waypoint in lane_center_waypoints
        )
        if not lane_exists:
            return 0
        ...
        for vehicle_snapshot in object_snapshots:
            ...
            if int(vehicle_lane_context.get("lane_id", 0)) != int(target_lane_id):
                continue
    ...
    def build_context(
        ...
        # Use the stored initial route (O(N) lookup) rather than a fresh
        # CARLA trace_route() call.  A fresh trace can return a DIFFERENT
        # path (e.g. RIGHT when the stored route says LEFT) because CARLA's
        # pathfinder is sensitive to the exact start waypoint/lane.  Using
        # get_current_route_info() ensures the prompt's route matches the
        # route the global planner is actually following.
        route_summary = planner.get_current_route_info(
            x_m=float(ego_x_m),
            y_m=float(ego_y_m),
        )
        lane_width_m = float(road_cfg.get("lane_width_m", 4.0))
        ego_lane_id = int(local_context.get("lane_id", 0))
        local_lane_ids = [
            int(lane_id)
        ...
        ego_lane_index = (
            local_lane_ids.index(int(ego_lane_id))
            if int(ego_lane_id) in local_lane_ids
            else -1
        )
        ...
        if bool(local_context.get("can_change_left", False)):
            left_lane_id = (
                local_lane_ids[int(ego_lane_index) + 1]
                if 0 <= int(ego_lane_index) < len(local_lane_ids) - 1
                else int(ego_lane_id)
            )
            lane_safe_left = self._lane_safe_flag(
                ...
                lane_center_waypoints=lane_center_waypoints,
                planner=planner,
                target_lane_id=int(left_lane_id),
                forward_longitudinal_safe_distance_m=forward_longitudinal_safe_distance_m,
                rear_longitudinal_safe_distance_m=rear_longitudinal_safe_distance_m,
            )
        lane_safe_right = 0
        if bool(local_context.get("can_change_right", False)):
            ...
            lane_safe_right = self._lane_safe_flag(
                ...
                lane_center_waypoints=lane_center_waypoints,
                planner=planner,
                target_lane_id=int(right_lane_id),
                forward_longitudinal_safe_distance_m=forward_longitudinal_safe_distance_m,
                rear_longitudinal_safe_distance_m=rear_longitudinal_safe_distance_m,
            )
