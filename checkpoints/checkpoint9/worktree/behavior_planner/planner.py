...
def intersection_route_follow_maneuver(
    ...
    maneuver_name = str(next_macro_maneuver or "").strip()
    normalized_maneuver_name = str(maneuver_name).upper()
    normalized_decision = str(decision or "LANE_KEEP").strip().upper()
    normalized_current_option = str(current_road_option or "").strip().upper()

    if mode_name != "INTERSECTION":
        return str(maneuver_name or "Continue Straight")
    if normalized_current_option in {"LEFT", "RIGHT"}:
        return "Lane Follow"
    if len(available_lane_ids) == 0:
        return str(maneuver_name or "Continue Straight")
    if normalized_decision != "LANE_KEEP":
        return str(maneuver_name or "Continue Straight")
    if (
        normalized_maneuver_name == "LEFT TURN"
        and int(target_lane_id) == int(max(available_lane_ids))
    ):
        return "Lane Follow"
    if (
        normalized_maneuver_name == "RIGHT TURN"
        and int(target_lane_id) == int(min(available_lane_ids))
    ):
        return "Lane Follow"
    return str(maneuver_name or "Continue Straight")
...
class RuleBasedBehaviorPlanner:
    ...
    def update(
        ...
        Returns
        -------
        {"decision": "LANE_KEEP" | "LANE_CHANGE_LEFT" | "LANE_CHANGE_RIGHT",
         "target_lane_id": int,
         "lc_state": str}
        """
        if len(lane_safety_scores) == 0:
            return self._make_result("LANE_KEEP", ego_lane_id)
        ...
        if self._lc_state != _IDLE and self._target_lane_id is not None:
            if self._lc_state == _CHANGING_LEFT:
                return self._make_result("LANE_CHANGE_LEFT", self._target_lane_id)
            else:
                return self._make_result("LANE_CHANGE_RIGHT", self._target_lane_id)
    ...
    def _intersection_target_lane_id(
        ...
        if maneuver_name == "LEFT TURN":
            return int(max(available_lane_ids))
        if maneuver_name == "RIGHT TURN":
            return int(min(available_lane_ids))
        if maneuver_name == "CONTINUE STRAIGHT":
            return None
        return int(ego_lane_id)
    ...
    def _start_one_step_lane_change(
        ...
        if int(desired_lane_id) > int(ego_lane_id):
            ...
            return self._make_result("LANE_KEEP", int(ego_lane_id))
    ...
    def _enter_lane_change_state(self, decision: str, target_lane_id: int) -> Dict[str, Any]:
        self._lc_state = (
            _CHANGING_LEFT if str(decision) == "LANE_CHANGE_LEFT" else _CHANGING_RIGHT
        )
        self._target_lane_id = int(target_lane_id)
        return self._make_result(str(decision), int(target_lane_id))
