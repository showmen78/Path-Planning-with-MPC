...
def run_loaded_world(client, world, scenario_cfg: Mapping[str, object], carla) -> int:
    ...
    current_target_v_mps = float(mpc.constraints.max_velocity_mps)
    ...
    active_reference_maneuver = str(
        getattr(initial_global_route_summary, "next_macro_maneuver", "Continue Straight")
    )
    ...
    lane_safety_scorer = LaneSafetyScorer(
        d_safe_m=float(rule_planner_cfg.get("d_safe_m", 8.0)),
        ttc_safe_s=float(rule_planner_cfg.get("ttc_safe_s", 3.0)),
        sigmoid_k=float(rule_planner_cfg.get("sigmoid_k", 5.0)),
        ttc_history_size=int(rule_planner_cfg.get("ttc_history_size", 10)),
    )
    lane_safety_thread = LaneSafetyThread(
        scorer=lane_safety_scorer,
        frequency_hz=float(rule_planner_cfg.get("lane_safety_hz", 10.0)),
    )
    rule_planner = RuleBasedBehaviorPlanner(
        hysteresis_delta=float(rule_planner_cfg.get("hysteresis_delta", 0.15)),
    )
    lane_safety_thread.start()
    print("[CARLA SCENARIO] Rule-based behavior planner thread started.")
    ...
    try:
        ...
        local_allowed_lane_ids = list(range(1, max(1, int(lane_count)) + 1))
        cached_obstacle_contours: List[dict] = []
        cached_hud_temp_lane_prompt = 1
        ...
        while True:
            ...
            if mpc.should_replan(sim_time_s):
                ...
                for obs in predicted_snapshots:
                    ...
                    lane_assignments[obs_id] = int(obs_ctx.get("lane_id", -1))

                # ---- Feed background threads ------------------------- #
                ego_transform = ego_vehicle.get_transform()
                lane_safety_thread.update_inputs(
                    ego_snapshot=ego_snapshot,
                    obstacle_snapshots=predicted_snapshots,
                    lane_assignments=lane_assignments,
                    ego_lane_id=int(current_lane_id),
                    available_lane_ids=local_allowed_lane_ids,
                    timestamp_s=sim_time_s,
                )
                # ---- Read cached thread results (non-blocking) ------- #
                lane_scores = lane_safety_thread.get_latest_scores()
                ...
                planning_next_maneuver = str(
                    getattr(planning_temporary_route_summary, "next_macro_maneuver", "Continue Straight")
                )
                raw_temp_mode_value, _temp_mode_road_id = compute_temp_destination_mode(
                    world_map=world_map,
                    carla=carla,
                    ...
                    intersection_threshold_m=float(
                        rule_planner_cfg.get("intersection_threshold_m", 30.0)
                    ),
                )
                temp_mode_str = (
                ...
                planner_output = rule_planner.update(
                    ...
                    route_optimal_lane_id=int(getattr(planning_temporary_route_summary, "optimal_lane_id", -1)),
                    next_macro_maneuver=str(planning_next_maneuver),
                )
                current_applied_behavior = str(planner_output.get("decision", "LANE_KEEP"))
                selected_lane_id = _clamp_lane_id_to_allowed(
                    int(planner_output.get("target_lane_id", current_lane_id)),
                ...
                lock_to_final_m = float(local_goal_cfg.get("lock_to_final_distance_m", 30.0))
                if dist_to_final <= lock_to_final_m:
                    # Close to final — lock destination to it
                    ...
                    temporary_destination_state = compute_temp_destination(
                        ...
                        global_route_points=initial_global_route_points,
                        mode_reference_xy=_prev_dest_xy,
                        prev_mode=_prev_mode,
                        prev_road_id=_prev_road_id,
                        next_macro_maneuver=str(active_planning_maneuver),
                        mode_override=str(temp_mode_str),
                    )
                ...
                # ---- MPC reference trajectory via CARLA waypoints ---- #
            ...
            if display is not None and topdown_queue is not None and chase_queue is not None:
                ...
                safety_str = " ".join(
                    f"L{lid}={s:.2f}"
                    for lid, s in sorted(lane_safety_thread.get_latest_scores().items())
                )
                ...
                hud_lines = [
                    ...
                    f"ego_lane={int(current_lane_id)}  sel_lane={int(selected_lane_id)}",
                    f"safety: {safety_str}",
                    f"mode={td_mode_str}  maneuver={str(active_reference_maneuver)}",
                    f"route_opt_lane={int(getattr(temporary_route_summary, 'optimal_lane_id', -1))}",
