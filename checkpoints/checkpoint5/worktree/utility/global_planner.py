...
class AStarGlobalPlanner:
    ...
    def nearest_waypoint_query(self, x_m: float, y_m: float) -> WaypointQueryResult | None:
        ...
        distance_m = math.hypot(float(nearest_node.x_m) - float(x_m), float(nearest_node.y_m) - float(y_m))
    ...
    def _route_waypoints_from_carla_trace(
        ...
        for carla_waypoint, road_option in route_trace:
            ...
            if lane_id is not None:
                ...
                elif not macro_maneuver_locked and int(lane_id) != int(preferred_lane_id):
                    preferred_lane_id = int(lane_id)

            option_name = str(getattr(road_option, "name", road_option)).upper()
            if macro_maneuver_locked:
                continue
        ...
        return route_waypoints, route_lane_ids, next_macro_maneuver, preferred_lane_id
    ...
    def _build_route_summary_from_carla_trace(
        ...
        route_waypoints, route_lane_ids, next_macro_maneuver, preferred_lane_id = self._route_waypoints_from_carla_trace(route_trace)
