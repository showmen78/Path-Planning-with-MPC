...
def compute_temp_destination(
    ...
    *entered_intersection* is ``1.0`` while the ego has entered the
    ...
    if is_intersection:
        # In intersection mode the blue dot stays tied to the global
        # route until the ego exits the junction.
        fallback_wp = _walk_forward(
            start_wp,
            float(lookahead_m),
            step_m,
            route_points=global_route_points if global_route_points and len(global_route_points) >= 2 else None,
            cum_dists=_route_cum_dists(global_route_points) if global_route_points and len(global_route_points) >= 2 else None,
        )
...
def build_reference_samples(
    ...
    route_points_valid = (
        global_route_points
        if is_intersection and global_route_points is not None and len(global_route_points) >= 2
        else None
    )
    ...
    if is_intersection and route_points_valid is not None:
