    ...
    def update(
        ...
        if self._lc_state != _IDLE and self._target_lane_id is not None:
            if self._lane_change_complete(
                ...
                self._lc_state = _IDLE
                self._selected_lane_id = int(self._target_lane_id)
                self._target_lane_id = None

        if self._lc_state != _IDLE and self._target_lane_id is not None and planner_mode == "INTERSECTION":
        ...
        if planner_mode == "INTERSECTION":
            ...
            if desired_lane_id is None or int(desired_lane_id) == int(current_selected_lane_id):
                return self._make_result(
                    _DECISION_FOLLOW,
