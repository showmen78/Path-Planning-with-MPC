...
intersection (not the ego's) plus the next global-route maneuver:
  ...
  **NORMAL**        (far from intersection)
      Advance along the global route geometry and, outside junctions,
      laterally align that route progress to the selected driving lane.
      This keeps the blue dot tied to the route while still allowing a
      temporary safe-lane detour.

  **INTERSECTION**  (near / inside intersection)
      Follow the global route through the junction, but do not
      autonomously snap into the route-optimal lane. Lateral lane
      changes still require an explicit behavior-planner decision.

In **both** modes the global route provides the longitudinal progress.
Outside intersections, the behaviour planner's selected lane decides
which adjacent lane the blue dot should align to.
...
def _follow_route_lane_for_decision(decision: str) -> bool:
    return str(normalize_behavior_decision(decision)) == "reroute"
        ...
        blended_samples.append(
            {
                ...
                "lane_id": int(
                    target_sample.get("lane_id", source_sample.get("lane_id", 1))
                    if alpha >= 0.5
                    else source_sample.get("lane_id", target_sample.get("lane_id", 1))
                ),
            }
        )
...
def compute_temp_destination(
    ...
    decision : str
        ...
        ``target_lane_id``; explicit lane-change decisions only affect
        how the reference starts transitioning from the ego lane.
    ...
    *entered_intersection* is ``1.0`` while the ego has entered the
    ...
    follow_route_lane = _follow_route_lane_for_decision(str(decision))
...
def build_reference_samples(
    ...
    follow_route_lane = _follow_route_lane_for_decision(str(normalized_decision))
    ...
    if route_points_valid is not None:
        source_samples = _build_route_reference_samples_from_anchor(
            world_map=world_map,
            carla=carla,
            anchor_wp=ego_wp,
            route_points=route_points_valid,
        ...
        target_samples = _build_route_reference_samples_from_anchor(
            world_map=world_map,
            carla=carla,
            anchor_wp=start_wp,
            route_points=route_points_valid,
