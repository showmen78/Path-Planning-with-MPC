...
# --------------------------------------------------------------------- #
class LaneSafetyThread:
    """Continuously update per-lane safety scores in a daemon thread."""
    ...
    def __init__(
        ...
        # Shared buffers (main-loop writes inputs, thread writes outputs)
    ...
    def get_latest_scores(self) -> Dict[int, float]:
        """Read latest safety scores (non-blocking)."""
        with self._lock:
    ...
    def _worker(self) -> None:
        ...
        while not self._stop_event.is_set():
            ...
            if data is not None:
                scores = self._scorer.compute_lane_scores(
                    ego_snapshot=data["ego_snapshot"],
                    obstacle_snapshots=data["obstacle_snapshots"],
                    lane_assignments=data["lane_assignments"],
                    ego_lane_id=data["ego_lane_id"],
                    available_lane_ids=data["available_lane_ids"],
                    timestamp_s=data["timestamp_s"],
                )
                # Clean up stale obstacle histories
                active = {
                    str(o.get("vehicle_id", ""))
                    for o in data["obstacle_snapshots"]
                }
                self._scorer.cleanup_stale_obstacles(active)
